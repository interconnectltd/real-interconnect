    ...
    function setupEventListeners() {
        ...
        document.addEventListener('click', (e) => {
            ...
            if (actionBtn) {
                ...
                // H3 Fix: 許可リストで実行可能な関数を制限（任意関数実行を防止）
                const allowedActions = ['viewProfile', 'viewConnection', 'viewEvent', 'viewMessage', 'viewMatching'];
                if (actionName && allowedActions.includes(actionName) && typeof window[actionName] === 'function') {
                    window[actionName]();
                } else if (actionName) {
                    console.warn('[NotificationsUnified] Blocked or unknown action:', actionName);
                }
            }
