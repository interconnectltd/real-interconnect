...
(function() {
    ...
    function setupEventListeners() {
        ...
        if (aiOnlyToggle) {
            aiOnlyToggle.addEventListener('change', function(e) {
        ...
        // URL ?highlight=USER_ID 対応
        var urlParams = new URLSearchParams(window.location.search);
        var highlightId = urlParams.get('highlight');
        if (highlightId) {
            setManagedTimeout(function() {
                var card = document.querySelector('.matching-card[data-user-id="' + highlightId + '"]');
                if (card) {
                    card.scrollIntoView({ behavior: 'smooth', block: 'center' });
                    card.classList.add('highlight-glow');
                    setTimeout(function() { card.classList.remove('highlight-glow'); }, 3000);
                }
            }, 1000);
        }
    }
    ...
    async function loadMatchingCandidates() {
        try {
            ...
            // ====== キャッシュ優先: matching_scores_cache + member_ai_profiles 取得 ======
            let cachedScoresMap = {};
            let aiProfileMap = {};
            ...
            try {
                ...
                const [cacheRes, aiRes] = await Promise.all([
                    window.supabaseClient
                        ...
                        .select('user_a_id, user_b_id, profile_score, transcript_score, interaction_score, total_score, match_reasons, is_stale')
                        .or('user_a_id.eq.' + currentUserId + ',user_b_id.eq.' + currentUserId)
                        .order('total_score', { ascending: false }),
                    window.supabaseClient
                        .rpc('get_public_ai_profiles')
                ]);

                if (cacheRes.data) {
                    for (const row of cacheRes.data) {
                        const otherId = row.user_a_id === currentUserId ? row.user_b_id : row.user_a_id;
                        cachedScoresMap[otherId] = row;
                    }
                }
                if (aiRes.data) {
                    for (const profile of aiRes.data) {
                        aiProfileMap[profile.user_id] = profile;
                    }
                }
                console.log('[MatchingUnified] キャッシュ取得完了:', {
                    cachedScores: Object.keys(cachedScoresMap).length,
                    aiProfiles: Object.keys(aiProfileMap).length
                });
            } catch (cacheErr) {
                console.warn('[MatchingUnified] キャッシュ取得エラー（フォールバック動作）:', cacheErr);
            }

            // user_profilesテーブルから全カラムを取得
            console.log('[MatchingUnified] user_profilesテーブルからデータ取得開始...');
            const { data: allUsers, error } = await window.supabaseClient
                .from('user_profiles')
                .select('id, name, full_name, company, position, bio, industry, location, skills, interests, business_challenges, picture_url, avatar_url, is_active, created_at')
                .eq('is_active', true)
                .limit(200);
    ...
    async function calculateMatchingScores(users) {
        try {
            ...
            const { data: currentUserData } = await window.supabaseClient
                .from('user_profiles')
                .select('id, skills, interests, business_challenges, industry, location')
                .eq('id', currentUserId)
                .maybeSingle();
            ...
            return users.map(user => {
                const cached = cachedScoresMap[user.id];
                const aiProfile = aiProfileMap[user.id];

                // ====== キャッシュが有効ならそれを使用 ======
                if (cached && !cached.is_stale) {
                    user.matchScore = Math.round(Number(cached.total_score));
                    user.hasAIData = Number(cached.transcript_score) > 0;
                    user.scoreBreakdown = {
                        profile: Number(cached.profile_score),
                        transcript: Number(cached.transcript_score),
                        interaction: Number(cached.interaction_score)
                    };
                    // match_reasons からラベル配列を生成
                    const reasonsData = cached.match_reasons || [];
                    user.matchReasons = reasonsData.map(function(r) { return r.label || ''; }).filter(Boolean);
                    user.matchReasonsData = reasonsData;
                } else {
                    // ====== フォールバック: クライアントサイド計算 ======
                    ...
                    if (complementarity.details.length > 0) {
                        complementarity.details.slice(0, 2).forEach(function(detail) {
                            reasons.push(detail.challenge + ': ' + detail.matchedSkills.slice(0, 2).join('、'));
                        });
                    }

                    var userSkills = Array.isArray(user.skills) ? user.skills :
                        (user.skills ? user.skills.split(',').map(function(s) { return s.trim(); }) : []);
                    var currentSkills = Array.isArray(currentUser.skills) ? currentUser.skills :
                        (currentUser.skills ? currentUser.skills.split(',').map(function(s) { return s.trim(); }) : []);

                    if (currentSkills.length > 0 && userSkills.length > 0) {
                        var commonSkills = currentSkills.filter(function(skill) { return userSkills.includes(skill); });
                        if (commonSkills.length > 0) {
                            score += Math.min((commonSkills.length / Math.max(currentSkills.length, 1)) * 20, 20);
                            reasons.push('共通スキル: ' + commonSkills.slice(0, 3).join('、'));
                        }
                    }

                    var userInterests = Array.isArray(user.interests) ? user.interests :
                        (user.interests ? user.interests.split(',').map(function(s) { return s.trim(); }) : []);
                    var currentInterests = Array.isArray(currentUser.interests) ? currentUser.interests :
                        (currentUser.interests ? currentUser.interests.split(',').map(function(s) { return s.trim(); }) : []);

                    if (currentInterests.length > 0 && userInterests.length > 0) {
                        var commonInterests = currentInterests.filter(function(i) { return userInterests.includes(i); });
                        if (commonInterests.length > 0) {
                            score += Math.min((commonInterests.length / Math.max(currentInterests.length, 1)) * 15, 15);
                    ...
                    user.hasAIData = false;
                    user.scoreBreakdown = null;
                    user.complementarityScore = complementarity;
                }

                // AIプロファイルメタデータを付与
                if (aiProfile) {
                    user.aiProfile = aiProfile;
                    user.meetingsAnalyzed = aiProfile.total_meetings_analyzed;
                }

                // 補完性スコアは常に計算（UI表示用）
                if (!user.complementarityScore) {
                    user.complementarityScore = calculateSkillChallengeMatch(currentUser, user);
                }
    ...
    function displayAIRecommendations() {
        var recContainer = document.getElementById('ai-recommendations-container');
        if (!recContainer) return;

        // AI分析済みのトップ3を抽出
        var aiUsers = matchingUsers
            .filter(function(u) { return u.hasAIData && u.matchScore >= 50; })
            .sort(function(a, b) {
                return (b.scoreBreakdown ? b.scoreBreakdown.transcript : 0) - (a.scoreBreakdown ? a.scoreBreakdown.transcript : 0);
            })
            .slice(0, 3);
        ...
        recContainer.innerHTML =
            '<div class="ai-rec-banner">' +
                '<h3 class="ai-rec-title"><i class="fas fa-robot"></i> AI\u304A\u3059\u3059\u3081\u30DE\u30C3\u30C1\u30F3\u30B0</h3>' +
                '<div class="ai-rec-cards">' +
                    aiUsers.map(function(u) {
                        var topReason = (u.matchReasonsData && u.matchReasonsData[0]) ? u.matchReasonsData[0].label : '';
                        return '<div class="ai-rec-card" data-user-id="' + u.id + '">' +
                            (u.picture_url ?
                                '<img src="' + sanitizeImageUrl(u.picture_url) + '" alt="" class="ai-rec-avatar">' :
                                '<div class="ai-rec-avatar-placeholder"><i class="fas fa-user"></i></div>') +
                            '<div class="ai-rec-info">' +
                                '<div class="ai-rec-name">' + escapeHtml(u.name || '') + '</div>' +
                                '<div class="ai-rec-company">' + escapeHtml(u.company || '') + '</div>' +
                                '<div class="ai-rec-score">' + u.matchScore + '%</div>' +
                                (topReason ? '<div class="ai-rec-reason">' + escapeHtml(topReason) + '</div>' : '') +
                            '</div>' +
                        '</div>';
                    }).join('') +
                '</div>' +
        ...
        // カードクリックでプロフィール表示
        recContainer.querySelectorAll('.ai-rec-card').forEach(function(card) {
    ...
    function createMatchingCard(user) {
        ...
        // AIバッジ（transcript_scoreが寄与している場合）
        const aiBadgeHTML = user.hasAIData ? '<div class="ai-matching-badge"><i class="fas fa-robot"></i> AI分析済み</div>' : '';

        // スコア内訳（キャッシュスコアがある場合）
        const breakdownHTML = user.scoreBreakdown ? (
            '<div class="score-breakdown-row">' +
                '<span title="プロフィール一致度" class="score-chip score-chip-profile">プロフィール ' + Math.round(user.scoreBreakdown.profile) + '</span>' +
                (user.scoreBreakdown.transcript > 0 ? '<span title="AI分析スコア" class="score-chip score-chip-ai">AI ' + Math.round(user.scoreBreakdown.transcript) + '</span>' : '') +
                (user.scoreBreakdown.interaction > 0 ? '<span title="交流実績" class="score-chip score-chip-interaction">交流 ' + Math.round(user.scoreBreakdown.interaction) + '</span>' : '') +
            '</div>'
        ) : '';

        // AI抽出の提供可能な価値
        const aiOfferingsHTML = (user.aiProfile && user.aiProfile.aggregated_offerings && user.aiProfile.aggregated_offerings.length > 0) ?
            '<div class="ai-offerings-box">' +
                '<span class="ai-offerings-label"><i class="fas fa-hand-holding-heart"></i> 提供可能な価値</span>' +
                '<div class="ai-offerings-tags">' +
                    user.aiProfile.aggregated_offerings.slice(0, 3).map(function(o) {
                        return '<span class="ai-offering-tag">' + escapeHtml(o) + '</span>';
                    }).join('') +
                '</div>' +
            '</div>' : '';

        // 分析実績
        const confidenceHTML = user.meetingsAnalyzed ?
            '<div class="ai-confidence-indicator"><i class="fas fa-chart-line"></i> ' + user.meetingsAnalyzed + '回のミーティングから分析</div>' : '';

        // マッチ理由（キャッシュのmatch_reasonsがある場合はそちらを優先）
        const matchReasonsDisplayHTML = (user.matchReasonsData && user.matchReasonsData.length > 0) ?
            '<div class="ai-match-reasons">' +
                user.matchReasonsData.slice(0, 2).map(function(r) {
                    return '<div class="ai-match-reason-item"><i class="fas fa-check-circle"></i> ' + escapeHtml(r.label || '') + '</div>';
                }).join('') +
            '</div>' : '';
        ...
        return `
            <div class="matching-card enhanced" data-user-id="${userId}" data-profile-id="${userId}" style="position: relative;">
                ...
                <!-- マッチ理由（AI or フォールバック） -->
                ${matchReasonsDisplayHTML || reasonsHTML}
    ...
    function filterUsers(users) {
        return users.filter(user => {
            ...
            // AI分析済みのみフィルター
            if (filters.aiOnly && !user.hasAIData) {
                return false;
            }
    ...
    function sortUsers(users) {
        ...
        switch (filters.sortBy) {
            ...
            case 'ai':
                // AI分析スコア降順 → 総合スコア降順
                sorted.sort((a, b) => {
                    const aiA = a.scoreBreakdown ? a.scoreBreakdown.transcript : 0;
                    const aiB = b.scoreBreakdown ? b.scoreBreakdown.transcript : 0;
