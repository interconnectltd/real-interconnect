...
window.InterConnect.Profile = {
            ...
            if (window.supabaseClient && this.currentUserId) {
                ...
                if (data && !error) {
                    ...
                    window.InterConnect.Profile.profileData = {
                        ...
                        email: data.email || window.InterConnect.Profile.profileData.email,
                        ...
                        title: data.position || window.InterConnect.Profile.profileData.position,
                        profileImage: data.avatar_url || window.InterConnect.Profile.profileData.profileImage,
                        coverImage: data.cover_url || window.InterConnect.Profile.profileData.coverImage,
                        ...
                        phone: data.phone || window.InterConnect.Profile.profileData.phone,
                        line_id: data.line_id || window.InterConnect.Profile.profileData.line_id,
                        transcript_analysis_consent: !!data.transcript_analysis_consent,
                        connectionCount: 0,
                        isOnline: data.is_online || false
                    };
                }
    ...
    openEditModal: function() {
        ...
        // AI同意チェックボックスを現在値に設定
        const aiConsentCheckbox = document.getElementById('edit-ai-consent');
        if (aiConsentCheckbox) {
            aiConsentCheckbox.checked = !!data.transcript_analysis_consent;
        }
    ...
    saveProfile: async function() {
        ...
        if (aiConsentCheckbox) window.InterConnect.Profile.profileData.transcript_analysis_consent = aiConsentCheckbox.checked;
        ...
        if (window.supabaseClient && this.currentUserId) {
            try {
                const updateData = {
                    ...
                    transcript_analysis_consent: !!window.InterConnect.Profile.profileData.transcript_analysis_consent,
                ...
                // マッチングキャッシュを無効化
                await window.supabaseClient.rpc('mark_matching_cache_stale', {
                    p_user_id: this.currentUserId
                }).catch(function(e) { console.warn('[Profile] キャッシュ無効化スキップ:', e); });

                // 同意撤回時はAI分析データを削除（GDPR対応）
                if (!updateData.transcript_analysis_consent) {
                    await window.supabaseClient.rpc('purge_ai_analysis_data', {
                        p_user_id: this.currentUserId
                    }).catch(function(e) { console.warn('[Profile] AI データ削除スキップ:', e); });
                }
            } catch (error) {
