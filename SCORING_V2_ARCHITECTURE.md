...
## 付録B: 実装状態（2026-04-10時点）

### 完了済み

| 項目 | ファイル | 内容 |
|---|---|---|
| DBテーブル作成 | 00005_scoring_v2.sql | user_conversation_vectors, matching_scores_v4, correction_log, feedback_log, scoring_config |
| RLSポリシー | 同上 | 全5テーブルに適用 |
| staleトリガー | 同上 | user_conversation_vectors更新→scores_v4 stale化 |
| scoring_config初期値 | 同上 | 動的重み(3段階) + alpha(6値) + ブースト(6パラメータ) |
| prompt_versions拡張 | 同上 | model_version, validated_accuracy, few_shot_count列追加 |
| v3.0.0プロンプト登録 | 同上 | Opus 4.6用マーカー（テンプレートはworkerにハードコード） |
| ユニーク制約 | SQL Editor手動 | transcript_insights(transcript_id, participant_id) |
| Zodバリデーション | analyze.ts内インライン | needs/offers/dynamics/topic/engagement/evidence/key_statementsスキーマ |
| Opus v3 analyzeハンドラー | worker/src/handlers/analyze.ts | model=opus-4-6, max_tokens=4096, v3プロンプト, Zodバリデーション+フォールバック, 会議品質係数 |
| aggregate V2フィールド保持 | worker/src/handlers/aggregate.ts | solver_profile/beneficiary_profile/explicit/signals/credibility/evidence保持 |
| テスト分析成功 | 手動実行 | 大石主歩: needs3/offers6/solver_profile有/beneficiary有/dynamics有 |

| Step 1.5: 既存21件のOpus再分析 | ✅ 完了（21件再分析完了） |
| Step 2: 集約エンジン | ✅ 完了（集約エンジン + user_conversation_vectors） |
| Step 3: 5次元スコア | ✅ 完了（5次元スコア + 動的重み + 推薦理由V2） |
| Step 4: フロントエンドV2 | ✅ 完了（フロントエンドV2切替） |
| Step 5: フィードバックUI | ✅ 完了（フィードバックUI実装（2段階モーダル + feedback_log記録）。暗黙シグナル集計は将来Phase。） |
| Step 6: V1クリーンアップ | ✅ 完了（V1コード削除（score-compute.ts, reason-templates.ts, compute/route.ts）） |

> **全Step完了** — V2スコアリング移行は全ステップ実装済み。

### 既知の課題

| 課題 | 詳細 |
|---|---|
| ~~全transcript_insightsがv2.0.0~~ | ~~v3.0.0で再分析されていない~~ → **解決済み（全21件再分析完了）** |
| ~~V1スコアリングが稼働中~~ | ~~matching_scores_v3を使用~~ → **解決済み（V2稼働中）** |
| Opusコスト$0.40/件 | 当初見積$0.10-0.15より高い（長いトランスクリプト+豊富な出力） |
| max_tokens=4096必要 | 3000ではJSONが切れる（solver/beneficiary_profile含むと出力が大きい） |
