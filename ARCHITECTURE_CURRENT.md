...
| ダッシュボード | 95% | 成熟度Lv表示済み |
| マッチング表示 | 90% | V1スコア稼働中、V2移行中 |
...
| AI分析（Opus v3） | 80% | コード完成、既存データ再分析待ち |
| V2スコアリング | 20% | テーブル作成済み、計算ロジック未実装 |
| 会議機能 | 40% | 一覧のみ、CRUD未完 |
| 設定ページ | 30% | プレースホルダー |
| フィードバックUI | 0% | Step 5 |
| AIプロフィール管理 | 0% | Step 4 |
...
| 1.5 | 既存21件の Opus 再分析 | **APIクレジット不足で中断** |
| 2 | Haiku重複判定 + user_conversation_vectors 並行書込 | 未着手 |
| 3 | 5次元スコア + 動的重み + Haiku判定バッチ | 未着手 |
| 4 | UI更新（/settings/ai-profile, FB UI） | 未着手 |
| 5 | フィードバック基盤 | 未着手 |
| 6 | V1クリーンアップ | 未着手 |
